class Field {
  ...
  translateDownfieldToX(yards, defendingGoal){
    if (RIGHT == defendingGoal){
      return this.maxX - yards * this.pxPerYard;
    } else {
      return this.minX + yards * this.pxPerYard;
    }
  }
  
  translateDistToCtrToY(yards, defendingGoal){
    if (RIGHT == defendingGoal){
      return this.midY - yards * this.pxPerYard;
    } else {
      return this.midY + yards * this.pxPerYard;
    }
  }
  ...
  rollTheField(){
    ...
    const numStripes = 18;
    const stripeWidth = this.lengthPx / numStripes;
    ...
    for (let i = 0; i < numStripes; i++){
      ...
      const stripeY = this.minY + i * stripeWidth;
      rect(this.minX + i * stripeWidth, this.minY, stripeWidth, this.widthPx);
    }
  }
  ...
  lineTheField(){
    ...
    const circleDiameter = 2 * (9.5 * this.pxPerYard);
