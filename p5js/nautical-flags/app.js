...
function setup() {
  ...
  if (window.mobileCheck()) {
    createCanvas(windowWidth, windowHeight * 0.50);

    mobileInput = createInput('');
    mobileInput.position(0, height + 15);
    mobileInput.size( 0.1 * width + 0.8 * width, 30);
    mobileInput.elt.focus();
  } else {
    createCanvas(windowWidth, windowHeight-35);
  }  
  P5JsSettings.init();

  nauticalFlags = new NauticalFlags( min(width * 0.6, height * 0.9) );
  showIntroScreen();
}
...
function showIntroScreen(){
  ...
  nauticalFlags.text('WELCOME', undefined, 0.1 * height);
